...
export default function PopularDestinations({ onReserveSpot }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      if (!containerRef.current || !targetRef.current || !pinRef.current)
        return;
      const totalWidth =
        containerRef.current.scrollWidth - window.innerWidth + 120;
  ...
  return (
    <section
      ref={targetRef}
      id="supplements-menu"
      className="relative min-h-screen bg-[#090C0E]"
    >
      <div
        ref={pinRef}
        className="relative h-screen w-full overflow-hidden flex flex-col justify-between py-8"
      >
        ...
        <div
          ref={containerRef}
          className="flex gap-8 px-6 md:px-12 items-center h-[73vh] z-10"
        >
          {supplementProducts.map((product) => {
            ...
            return (
              <ProductCard
                ...
                price={product.price}
                originalPrice={
                  product.price ? Math.round(product.price * 1.25) : null
                }
        ...
        <div className="px-6 md:px-12 flex items-center justify-between z-10 shrink-0">
          <div className="flex items-center gap-4 flex-1 max-w-md">
            <span className="text-xs font-mono text-[#8A94A0] uppercase">
              SWIPE ENGINE
            </span>
                ...
                {cart.length === 0 ? (
                  ...
                  <div className="space-y-4 max-h-[55vh] overflow-y-auto pr-2 custom-scrollbar">
                    {cart.map((item) => (
                      <div
                        key={item.id}
                        className="p-4 rounded-2xl bg-[#090C0E] border border-white/10 flex items-center justify-between gap-3"
                      >
                        <div className="flex-1">
                          <h4 className="text-xs font-bold text-white uppercase line-clamp-1">
                            {item.name}
                          </h4>
                          <span className="text-xs font-mono text-[#FF2E4C]">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="w-7 h-7 rounded-lg bg-[#12161A] text-white flex items-center justify-center"
                          >
                            <Minus size={12} />
                          </button>

...
export default function ParallaxGallery({ onClaimTrial }) {
  ...
  return (
    <section ref={containerRef} className="py-24 px-4 md:px-12 bg-[#090C0E] overflow-hidden relative border-t border-white/10">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-6xl font-black font-heading text-white uppercase tracking-tight">
            PHYSICAL <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF2E4C] via-[#FF526B] to-[#00F0FF]">DOMINANCE</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-20 min-h-[600px]">
          ...
          <motion.div style={{ y: y1 }} className="flex flex-col gap-6">
            {col1Images.map((src, i) => (
              <div key={i} className="relative h-80 rounded-3xl overflow-hidden bg-[#12161A] border border-white/10 group shadow-2xl">
                <img src={src} alt="Parallax 1" className="w-full h-full object-cover group-hover:scale-108 transition-transform duration-700 opacity-80" />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
                <div className="absolute bottom-6 left-6 text-lg font-bold font-heading text-white tracking-wide">PURE FORCE</div>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div style={{ y: y2 }} className="flex flex-col gap-6 -mt-12">
            {col2Images.map((src, i) => (
              <div key={i} className="relative h-[360px] rounded-3xl overflow-hidden bg-[#12161A] border border-[#FF2E4C]/30 group shadow-2xl">
                <img src={src} alt="Parallax 2" className="w-full h-full object-cover group-hover:scale-108 transition-transform duration-700 opacity-80" />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
                <div className="absolute bottom-6 left-6 text-lg font-bold font-heading text-[#FF2E4C] tracking-wide">TITAN WILL</div>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div style={{ y: y3 }} className="flex flex-col gap-6">
            {col3Images.map((src, i) => (
              <div key={i} className="relative h-80 rounded-3xl overflow-hidden bg-[#12161A] border border-white/10 group shadow-2xl">
                <img src={src} alt="Parallax 3" className="w-full h-full object-cover group-hover:scale-108 transition-transform duration-700 opacity-80" />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
                <div className="absolute bottom-6 left-6 text-lg font-bold font-heading text-[#00F0FF] tracking-wide">NEVER RETREAT</div>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div style={{ y: y4 }} className="flex flex-col gap-6 -mt-16">
            {col4Images.map((src, i) => (
              <div key={i} className="relative h-[380px] rounded-3xl overflow-hidden bg-[#12161A] border border-[#FF2E4C]/30 group shadow-2xl">
                <img src={src} alt="Parallax 4" className="w-full h-full object-cover group-hover:scale-108 transition-transform duration-700 opacity-80" />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
                <div className="absolute bottom-6 left-6 text-lg font-bold font-heading text-[#FF2E4C] tracking-wide">RAW KINETICS</div>
              </div>
            ))}
          </motion.div>

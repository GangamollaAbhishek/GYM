...
const DEFAULT_CARDS = [
  {
    ...
    toast:
      "Hypertrophic Heavy Lifting Protocol booked! Biometric scanner ready.",
    accent: CRIMSON,
  },
  {
    key: "cardio",
  ...
  return (
    <span
      ...
      style={{
        ...
        color,
        backgroundColor:
          tone === "crimson" ? `${CRIMSON}15` : "rgba(0,0,0,0.4)",
      }}
    >
      <ArrowRight size={16} />
    </span>
...
export default function ExploreEscape({ onReserveSpot }) {
  ...
  const CARDS =
    eeData.cards && eeData.cards.length > 0
      ? eeData.cards.map((c, i) => ({
          ...c,
  ...
  return (
    <section
      id="explore-escape"
      className="relative z-10 overflow-hidden bg-[#090C0E] px-4 py-16 md:px-12 md:py-24 border-t border-white/10"
    >
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        ...
        <div className="mb-12 flex items-start justify-between md:mb-16">
          <div>
            <p className="text-xs font-mono font-bold tracking-[0.2em] uppercase text-[#00F0FF] flex items-center gap-2">
              ...
              {eeData.tagline || "EXPLORE THE UNSEEN"}
            </p>
            <h2 className="mt-3 text-4xl font-black font-heading leading-[1.1] text-white uppercase md:text-6xl tracking-tight">
              {eeData.headingMain || "Find your next"}{" "}
              <br className="hidden md:block" />
              <em className="italic font-normal lowercase text-[#FF2E4C] font-serif">
                {eeData.headingHighlight || "breaking point."}
              </em>
            </h2>
          </div>
        ...
        <p className="mt-8 text-right text-xs font-mono uppercase tracking-[0.2em] text-[#8A94A0] md:mt-10">
          TRAIN HARD.{" "}
          <span className="text-[#FF2E4C] font-bold">EXPLORE LIMITS.</span>{" "}
          DOMINATE.
        </p>
      </div>
    </section>

...
/* ──────────────────────────────────────────────────────
   Niche: AI Agent Workspace & Admin Intelligence
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "bg-[#13141b] border border-white/[0.07] hover:border-white/[0.14]",
        "shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05),0_0_0_1px_rgba(255,255,255,0.02),0_4px_16px_rgba(0,0,0,0.5)]",
    ...
    >
      <div className="z-10 flex flex-col gap-1">
        <div className="flex items-center justify-between">
          ...
          <span className="w-1.5 h-1.5 rounded-full bg-[#FF2E4C] opacity-70 group-hover:opacity-100 transition-opacity shadow-[0_0_8px_#FF2E4C]" />
        </div>
        <p className="text-slate-400 text-xs leading-relaxed max-w-[92%] font-normal">
          {description}
        </p>
      </div>
      <div className="relative mt-2 flex-1 w-full rounded-[14px] overflow-hidden border border-white/[0.06] bg-[#0c0e13]/90">
        {children}
      </div>
    </motion.div>
  );
...
export function Card1() {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden select-none bg-[#0a0c10] rounded-xl flex items-center justify-center p-2">
      {/* ── Layer 1: Clean dotted grid ── */}
      <svg className="absolute inset-0 w-full h-full" aria-hidden>
        <defs>
          <pattern id="clean-grid-admin" width="16" height="16" patternUnits="userSpaceOnUse">
            <circle cx="1.5" cy="1.5" r="0.75" fill="currentColor" className="text-zinc-800/80" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#clean-grid-admin)" />
      </svg>
                ...
                {isBox && node.icon ? (
                  ...
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shadow-sm transition-all duration-300 ${isActive
                      ...
                      : "bg-[#14161f] border-zinc-700"
                      }`}
                  >
                    <motion.div
                      className={`w-2.5 h-2.5 rounded-full border border-dashed ${isActive ? "border-amber-500" : "border-zinc-500"
                        }`}
                      animate={{ rotate: 360 }}
...
export function Card2() {
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between p-1">
        ...
        {[
          ...
          return (
            <div key={i} className="flex-1 h-[76px] relative select-none">
              ...
              <div
                className="absolute inset-0 rounded-xl border border-white/[0.06] bg-white/[0.02] text-white/[0.08]"
                style={{
              ...
              <motion.div
                className="absolute inset-0 w-full h-full rounded-xl bg-[#14161f] border border-white/[0.1] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.04)] p-3 hover:bg-[#181a24] transition-colors duration-300 backdrop-blur-[2px] flex items-center justify-between gap-3 cursor-pointer"
                animate={{
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[8px] text-slate-400 font-mono uppercase tracking-widest leading-none">{s.label}</span>
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-[8px] text-slate-500 font-mono">prev</span>
                  </div>
                </div>
                  ...
                  <svg className="w-full h-full overflow-visible" viewBox="0 0 48 24">
                    ...
                    {(i === 0
                      ...
                      <motion.circle
                        ...
                        cy={pt.y}
                        r="1.5"
                        className="fill-[#14161f] stroke-slate-400"
                        strokeWidth="1"
      ...
      {/* Bar chart */}
      <div className="flex-1 flex items-end gap-2 px-0.5 min-h-[90px]">
        {bars.map((h, i) => (
          <div
            key={i}
            className="flex-1 h-full rounded-xl bg-[#0a0c10] border border-white/[0.08] relative overflow-hidden text-white/[0.06]"
            style={{
              backgroundImage: "repeating-linear-gradient(45deg, transparent, transparent 6px, currentColor 6px, currentColor 7px)",
            }}
          >
      ...
      <div className="flex gap-2 px-0.5">
        {days.map((d, i) => (
          <p key={i} className="flex-1 text-center text-[8px] text-slate-400 font-mono font-medium">{d}</p>
        ))}
      </div>
    </div>
  );
...
const STATUS_ICONS = {
  done: { icon: Check, color: "text-lime-400", bg: "bg-lime-500/15", gradient: "bg-gradient-to-b from-lime-400 to-lime-600", border: "border-lime-600" },
  ...
  idle: { icon: Minus, color: "text-slate-400", bg: "bg-white/[0.05]", gradient: "bg-gradient-to-b from-zinc-400 to-zinc-600", border: "border-zinc-600" },
};
        ...
        return (
          ...
          >
            <div className={`w-full rounded-2xl border flex items-center gap-2.5 ${isActive
              ? "px-3 py-2.5 bg-[#141620] border-white/[0.12] shadow-lg"
              : "px-2.5 py-1.5 bg-[#0e1017]/80 border-white/[0.05]"
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className={`font-mono font-semibold text-white leading-none ${isActive ? "text-[10px]" : "text-[9px]"
                    }`}>{l.agent}</span>
                  <span className={`font-mono uppercase tracking-wide rounded px-1 py-0.5 ${si.bg} ${si.color} ${isActive ? "text-[7px]" : "text-[6px]"
                    }`}>{l.status}</span>
                </div>
                {isActive && (
                  <p className="text-[9px] text-slate-300 truncate mt-0.5 leading-tight font-normal">{l.action}</p>
                )}
              </div>

              {isActive && (
                <span className="text-[9px] font-mono text-slate-400 shrink-0">{l.t}</span>
              )}
            </div>
          </motion.div>
        );
  ...
  return (
    <div className="w-full h-full flex flex-col md:flex-row gap-4 py-2 px-3">
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 pr-1">
        <p className="text-[8px] font-mono uppercase tracking-widest text-slate-400 mb-3">Namespaces</p>
          ...
          {namespaces.map((ns, i) => {
            ...
            return (
              <div key={ns.name} className="flex items-center gap-3 group relative">
                ...
                <div
                  className={`relative flex shrink-0 items-center justify-center w-[34px] h-[34px] rounded-[10px] border transition-all duration-500 ${isActive ? `shadow-[inset_0_1px_0_0_rgba(255,255,255,0.4),inset_4px_4px_0_0_rgba(255,255,255,0.06),inset_6px_6px_0_0_rgba(255,255,255,0.04),inset_8px_8px_0_0_rgba(255,255,255,0.02),0_1px_2px_0_rgba(0,0,0,0.08),0_2px_4px_0_rgba(0,0,0,0.06),0_4px_6px_0_rgba(0,0,0,0.04),0_6px_8px_0_rgba(0,0,0,0.02)] text-white ${c.buttonBg} ${c.buttonBorder} scale-105` : 'bg-[#14161f] border-white/[0.06] text-slate-400'}`}
                >
                  <Icon size={16} weight={isActive ? "fill" : "regular"} className="relative z-10" />
                </div>

                {/* Name */}
                <span className={`text-[10px] font-mono w-16 shrink-0 transition-colors duration-300 ${isActive ? "text-white font-semibold" : "text-slate-400 group-hover:text-slate-200"}`}>
                  {ns.name}
                </span>

                {/* Cyberpunk Bar track */}
                <div className="flex-1 h-1.5 bg-white/[0.06] rounded-full overflow-hidden relative backdrop-blur-sm shadow-inner">
                  <motion.div
                ...
                <div className={`flex items-center gap-1.5 w-10 justify-end transition-all duration-500 ${isActive ? "opacity-100 scale-105" : "opacity-60 scale-100"}`}>
                  <span className={`text-[9px] font-mono font-medium ${isActive ? "text-white" : "text-slate-400"}`}>
                    {ns.hits}
                  </span>
                  {isActive && (
                    <motion.div
        ...
        <div className="flex items-center gap-2 pt-2.5 mt-auto">
          ...
          <span className="text-[8px] font-mono text-slate-400 font-medium tracking-wide">Live retrieval active</span>
        </div>
      </div>

      {/* Thin divider */}
      <div className="hidden md:block w-px bg-white/[0.08] self-stretch shrink-0" />

      {/* ── Right panel: Retrieval log ── */}
      <div className="w-full md:w-[172px] shrink-0 flex flex-col gap-0">
        <p className="text-[8px] font-mono uppercase tracking-widest text-slate-400 mb-2.5">Retrieval Log</p>
        ...
        <div className="flex flex-col gap-1.5 flex-1 overflow-hidden">
          {recentQueries.map((q, qi) => {
            ...
            return (
              <motion.div
                key={`${q.ns}-${q.q}-${qi}`}
                className="rounded-xl border border-white/[0.08] bg-[#141620] px-2.5 py-1.5 shadow-sm"
              ...
              >
                <div className="flex items-center gap-1 mb-0.5">
                  ...
                  <span className="text-[7px] font-mono text-slate-500 ml-auto tabular-nums">{q.t}</span>
                </div>
                <p className="text-[8px] text-slate-300 leading-tight font-mono truncate">{q.q}</p>
              </motion.div>
            );
          })}
        </div>
    ...
    <div className="w-full h-full flex items-center justify-center p-1.5">
      <div className="grid grid-cols-2 gap-2 w-full">
        {tools.map((t, i) => (
          <motion.div
            key={i}
            className="relative rounded-[14px] border border-white/[0.08] bg-[#141620] shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between p-2 group hover:border-white/[0.15]"
          ...
          >
            ...
            <div className="flex items-start justify-between">
              <div className={`w-[26px] h-[26px] rounded-[7px] flex items-center justify-center text-white ${t.color} border ${t.borderColor} shadow-[inset_0_0.5px_0_0_rgba(255,255,255,0.6),inset_0.5px_0_0_0_rgba(255,255,255,0.2),inset_0_2px_6px_0_rgba(255,255,255,0.3),inset_0_-0.5px_0_0_rgba(0,0,0,0.3),inset_-0.5px_0_0_0_rgba(0,0,0,0.1),inset_0_-2px_6px_0_rgba(0,0,0,0.1),0_1px_2px_0_rgba(0,0,0,0.08),0_2px_4px_0_rgba(0,0,0,0.06),0_4px_6px_0_rgba(0,0,0,0.04)] group-hover:scale-105 transition-transform duration-300`}>
              ...
              <div className="flex flex-col items-end gap-0.5">
                ...
                <span className="text-[7px] font-mono text-slate-400 uppercase tracking-widest leading-none">Calls</span>
              </div>
            </div>
            ...
            <div className="mt-1.5 flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <span className="text-[9.5px] font-mono font-medium text-slate-200 tracking-tight">{t.name}</span>
                <span className="text-[7.5px] font-mono text-slate-400 tabular-nums">{t.latency}</span>
              </div>
              <div className="w-full h-1 bg-white/[0.08] rounded-full overflow-hidden shadow-inner relative">
                <motion.div

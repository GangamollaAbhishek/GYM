...
export default function AuthPage({ onAuthSuccess }) {
  ...
  // Sync panel state based on route change without wiping user input
  useEffect(() => {
    const isSignUp =
      location.pathname === "/signup" || location.pathname === "/register";
  ...
  // Track if user is actively submitting auth form to prevent premature redirection/resets
  const isSubmittingRef = useRef(false);

  // Auto-redirect if already logged in on initial page mount (e.g., opened URL while already authenticated)
  useEffect(() => {
    if (
      isAuthenticated &&
      user &&

...
export default function TransitionScribble() {
    useEffect(() => {
        ...
        // ONLY RED COLOR for Titan Gym Scribble Transition
        const gymRedColor = '#E50914';
        ...
        const runScribbleAnimation = (e) => {
            ...
            // Set strictly red color
            transitionScribbleSvg.style.color = gymRedColor;
            ...
            if (!transitionLogo) {
                ...
                transitionLogo.style.cssText = 'position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); z-index:100000; pointer-events:none; opacity:0; display:flex; justify-content:center; align-items:center; transition: color 0.1s;';
                
                const logoSource = document.querySelector('.logo-truus');
                if (logoSource) {
                    const svgClone = logoSource.cloneNode(true);
                    svgClone.style.width = '320px';
                    svgClone.style.height = 'auto';
                    transitionLogo.appendChild(svgClone);
                } else {
                    const logoFallback = document.createElement('div');
                    logoFallback.className = 'flex items-center gap-3 sm:gap-5 font-bebas text-6xl sm:text-8xl md:text-9xl lg:text-[120px] font-black tracking-[0.18em] uppercase text-white drop-shadow-[0_15px_35px_rgba(0,0,0,0.9)]';
                    logoFallback.innerHTML = `<span>TITAN</span><span style="color: #E50914;">•</span><span>PULSE</span>`;
                    transitionLogo.appendChild(logoFallback);
                }
            ...
            gsap.set(transitionLogo, { opacity: 0, scale: 1.2 });
            ...
            drawTl.to(transitionLogo, {
                ...
                onStart: () => {
                    const logoChild = transitionLogo.querySelector('svg') || transitionLogo.querySelector('div');
                    if (logoChild) {
                        gsap.to(logoChild, { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                    }
                }
            }, durIn * 0.5);
            ...
            drawTl.set(transitionLogo, {
                ...
                onComplete: () => {
                    const logoChild = transitionLogo.querySelector('svg') || transitionLogo.querySelector('div');
                    if (logoChild) {
                        gsap.killTweensOf(logoChild);
                        gsap.set(logoChild, { rotation: 0 });
                    }
                }
            }, durIn + (durOut * 0.48));
        };
        ...
        const handleGlobalClick = (e) => {
            const logoElem = e.target.closest('.logo-truus');
            if (logoElem) {
                runScribbleAnimation(e);
            }
        };

        document.addEventListener('click', handleGlobalClick);

        // Auto-run on load / before hero section appears

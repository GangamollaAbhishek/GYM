    ...
    const runScribbleAnimation = (e) => {
      ...
      if (!transitionLogo) {
        ...
        transitionLogo.style.cssText =
          "position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); z-index:100000; pointer-events:none; opacity:0; display:flex; justify-content:center; align-items:center; transition: color 0.1s;";
        ...
        if (logoSource) {
          ...
          logoFallback.className =
            "flex items-center gap-3 sm:gap-5 font-bebas text-6xl sm:text-8xl md:text-9xl lg:text-[120px] font-black tracking-[0.18em] uppercase text-white drop-shadow-[0_15px_35px_rgba(0,0,0,0.9)]";
      ...
      drawTl.to(
        transitionLogo,
        {
          ...
          onStart: () => {
            const logoChild =
              transitionLogo.querySelector("svg") ||
              transitionLogo.querySelector("div");
            if (logoChild) {
              gsap.to(logoChild, {
                rotation: 5,
                duration: 0.15,

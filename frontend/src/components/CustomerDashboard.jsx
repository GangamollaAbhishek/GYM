...
export default function CustomerDashboard({ onLogout }) {
  ...
  const showToast = (msg, type = "info") => {
    ...
    const isGate =
      typeof msg === "string" &&
      (msg.includes("⚡") || msg.includes("Gate") || msg.includes("Turnstile"));
  ...
  const [phone, setPhone] = useState(
    user?.phone && user.phone !== "N/A" ? user.phone : "+91 98765 43210",
  );
  const [address, setAddress] = useState(
    user?.address && typeof user.address === "object"
      ? {
          street:
            user.address.street ||
            "Flat 402, Titan Heights, Road No. 36, Jubilee Hills",
  ...
  const [coachingData, setCoachingData] = useState({
    ...
    trainerNotes:
      user?.trainerNotes && user.trainerNotes.length > 0
        ? user.trainerNotes
        : [
            {
  ...
  const handleAvatarFileChange = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Cloudinary upload warning, using direct secure storage:",
          err,
        );
      }
      ...
      try {
        const storedUser = JSON.parse(
          localStorage.getItem("titan_user") || "{}",
        );
        localStorage.setItem(
          "titan_user",
          JSON.stringify({ ...storedUser, avatar: finalUrl }),
        );
      } catch (err) {
        console.warn(err);
      }
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      showToast(
        "✓ All profile details & physical telemetry saved successfully in MongoDB Atlas!",
      );
    } catch (err) {
      console.error("Update profile error:", err);
      setIsEditingProfile(false);
      showToast(
        err.response?.data?.message || "✓ Profile information updated!",
      );
    }
  };
  ...
  const [localMembershipStatus, setLocalMembershipStatus] = useState(
    () => user?.membershipStatus || "No Membership",
  );
  const [localStartDate, setLocalStartDate] = useState(
    () => user?.membershipStartDate || "",
  );
  const [localExpiryDate, setLocalExpiryDate] = useState(
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.membershipStatus)
        setLocalMembershipStatus(user.membershipStatus);
      ...
      if (user.amountPaid !== undefined && user.amountPaid !== null)
        setLocalAmountPaid(user.amountPaid);
  ...
  const activePlanName = hasActiveMembership
    ? membershipPlan
    : "No Active Plan";
  ...
  const getRemainingDaysInfo = () => {
    ...
    try {
      ...
      return {
        text: `${diffDays} Days Remaining (${percent}% Duration)`,
        percentage: percent,
        daysLeft: diffDays,
        isExpired: false,
      };
  ...
  const ordersList = useMemo(() => {
    ...
    if (Array.isArray(storeOrders)) {
      storeOrders.forEach((ord) => {
        ...
        list.push({
          ...
          paymentStatus: ord.paymentMethod
    ...
    if (hasActiveMembership || amountPaid || user?.amountPaid) {
      const ordDate =
        localStartDate ||
        (user?.membershipStartDate
          ? new Date(user.membershipStartDate).toLocaleDateString("en-GB", {
              day: "2-digit",
              month: "short",
              year: "numeric",
            })
      ...
      const ordId =
        user?._id || user?.id
          ? `ORD-${String(user._id || user.id)
              .slice(-6)
              .toUpperCase()}`
          : "ORD-892144";
      const planTitle =
        localMembershipPlan || user?.membershipPlan || "Gym Membership Pass";
      const methodStr = user?.paymentMethod
        ? `Paid (${user.paymentMethod})`
        : "Paid (Card / Online)";
      const amountStr =
        amountPaid || user?.amountPaid
          ? `₹${Number(amountPaid || user?.amountPaid).toLocaleString("en-IN")}`
          : "₹2,499";
      ...
      list.push({
        ...
        items:
          "24/7 Floor Access • Biometric Smart Locker • 3D Telemetry Audit • Sauna & Recovery",
      });
    }
  ...
  const filteredOrders =
    orderFilter === "all"
      ? ordersList
      : ordersList.filter((o) =>
          o.category.toLowerCase().includes(orderFilter.toLowerCase()),
        );
  ...
  const DEFAULT_LANDING_PLANS = [
    ...
    },
    {
      ...
      popular: false,
      description:
        "VIP priority lounge, cryotherapy chambers, hydro-massage therapy suite, custom micro-nutrient bar access, and unlimited guest privileges.",
      features: [
    ...
    },
    {
      ...
      popular: false,
      description:
        "Dedicated Master Personal Trainer, tailored meal plans, weekly 3D muscle bio-scans, dynamic heart-rate telemetry, and 24/7 direct coach line.",
      features: [
  ...
  const membershipPlans =
    rawMembershipsList && rawMembershipsList.length > 0
      ? rawMembershipsList.map((m, idx) => {
          const fallback =
            DEFAULT_LANDING_PLANS[idx] || DEFAULT_LANDING_PLANS[0];
          const rawServices = Array.isArray(m.services)
            ? m.services
            : m.services && typeof m.services === "object"
          ...
          const activeFeatures = activeServices
          ...
          return {
            ...
            price:
              typeof m.price === "number"
                ...
                : m.price || fallback.price,
            rawPrice:
              typeof m.price === "number"
                ? m.price
                : parseInt(
                    String(m.price || fallback.price).replace(/[^\d]/g, ""),
                    10,
                  ) || 2499,
            ...
            features:
              activeFeatures.length > 0
                ? activeFeatures
                : m.perks
                  ? m.perks.split(",").map((s) => s.trim())
                  : fallback.features,
          };
        })
      : DEFAULT_LANDING_PLANS.map((p) => ({
          ...p,
          services: p.features.map((f, i) => ({
            id: `srv-${i}`,
            name: f,
            category: "Facility Access",
            included: true,
      ...
      (p) =>
        (membershipPlan &&
          ...
          p.name.toLowerCase().trim() ===
            membershipPlan.toLowerCase().trim()) ||
        (membershipPlan &&
          p.tierKey &&
          membershipPlan.toLowerCase().includes(p.tierKey.toLowerCase())) ||
        (membershipPlan &&
          p.id &&
          membershipPlan.toLowerCase().includes(p.id.toLowerCase())),
    ) || membershipPlans[0];
  ...
  const validateCardPayment = () => {
    ...
    if (!cardExpiry) {
      ...
      if (m < 1 || m > 12) {
        ...
        const currentYearTwoDigits = parseInt(
          new Date().getFullYear().toString().slice(-2),
          10,
        );
        const currentMonth = new Date().getMonth() + 1;
        if (
          y < currentYearTwoDigits ||
          (y === currentYearTwoDigits && m < currentMonth)
        ) {
  ...
  const completeMembershipActivation = async (
    planName,
    priceNum,
    paymentMode = "Card",
  ) => {
    try {
      ...
      try {
        ...
        if (verifyRes.data?.data) {
          if (verifyRes.data.data.membershipExpiry)
            expiryDateStr = verifyRes.data.data.membershipExpiry;
          if (verifyRes.data.data.startDate)
            startDateStr = verifyRes.data.data.startDate;
        }
      } catch (e) {
      ...
      showToast(
        `🎉 Payment Confirmed via ${paymentMode}! ${planName} is now Active until ${expiryDateStr}.`,
      );
  ...
  const handleBuyPlan = (plan) => {
    ...
    const priceNum =
      plan.rawPrice ||
      parseInt(String(plan.price).replace(/[^\d]/g, ""), 10) ||
      2499;
  ...
  const myAssignedTrainer = useMemo(() => {
    ...
    if (user?.assignedTrainer) {
      const found = realTrainers.find(
        (t) =>
          t.id === String(user.assignedTrainer) ||
          t.id === user.assignedTrainer,
      );
    ...
    if (user?.assignedTrainerName) {
      const found = realTrainers.find(
        (t) => t.name.toLowerCase() === user.assignedTrainerName.toLowerCase(),
      );
      if (found) return found;
      return {
        id: "assigned-1",
  ...
  const handleAssignTrainer = async (trainer) => {
    try {
      ...
      if (!hasMembership) {
        showToast(
          "⚠️ Please purchase a gym membership plan first to get an assigned coach.",
        );
        return;
      }
  ...
  const allTransactions = useMemo(() => {
    ...
    if (Array.isArray(storeOrders)) {
      storeOrders.forEach((ord) => {
        ...
        list.push({
          ...
          rawAmount: amountNum,
          status:
            ord.status && ord.status.toLowerCase().includes("pending")
              ? "Pending Token"
              : "Success",
          orderDetails: ord,
        });
    ...
    if (
      ...
      const matchedPlan = membershipPlans.find(
        (p) =>
          (localMembershipPlan &&
            p.name &&
            p.name.toLowerCase().trim() ===
              localMembershipPlan.toLowerCase().trim()) ||
          (localMembershipPlan &&
            p.tierKey &&
            localMembershipPlan
              .toLowerCase()
              .includes(p.tierKey.toLowerCase())) ||
          (localMembershipPlan &&
            p.id &&
            localMembershipPlan.toLowerCase().includes(p.id.toLowerCase())),
      );
      ...
      const txId =
        user?._id || user?.id
          ? `TXN-MEM-${String(user._id || user.id)
              .slice(-6)
              .toUpperCase()}`
          : "TXN-MEM-892144";
      const planTitle =
      ...
      const priceNum =
        ...
        user?.amountPaid ||
        (matchedPlan
          ? matchedPlan.rawPrice ||
            parseInt(String(matchedPlan.price).replace(/[^\d]/g, ""), 10)
          : 2499);
  ...
  const membershipPayments = useMemo(() => {
    ...
    const invId =
      user?._id || user?.id
        ? `INV-MEM-${String(user._id || user.id)
            .slice(-6)
            .toUpperCase()}`
        : "INV-MEM-892144";
    const planTitle =
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return allTransactions.filter((t) =>
      t.category.toLowerCase().includes(paymentFilter.toLowerCase()),
    );
  ...
  const activeWorkoutSplits = useMemo(() => {
    if (
      coachingData.workoutPlan?.dailySplits &&
      typeof coachingData.workoutPlan.dailySplits === "object" &&
      Object.keys(coachingData.workoutPlan.dailySplits).length > 0
    ) {
      return coachingData.workoutPlan.dailySplits;
    }
    const currentSplitName =
      coachingData.workoutPlan?.split || "Push-Pull-Legs (Hypertrophy)";
    return (
      DEFAULT_WORKOUT_SPLITS[currentSplitName] ||
      DEFAULT_WORKOUT_SPLITS["Push-Pull-Legs (Hypertrophy)"]
    );
  }, [coachingData.workoutPlan]);

  const safeWorkoutDay = activeWorkoutSplits[workoutDay]
    ? workoutDay
    : Object.keys(activeWorkoutSplits)[0] || "day1";
  const currentDayExercises =
    activeWorkoutSplits[safeWorkoutDay]?.exercises || [];
  const completedCount = currentDayExercises.filter(
    (ex) => completedExercises[ex.id],
  ).length;
  const progressPercent = Math.round(
    (completedCount / (currentDayExercises.length || 1)) * 100,
  );
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      const savedGlobal = JSON.parse(
        localStorage.getItem("titan_global_feedbacks") || "[]",
      );
      localStorage.setItem(
        "titan_global_feedbacks",
        JSON.stringify([fbPayload, ...savedGlobal]),
      );
      await api.post("/feedbacks", fbPayload);
      showToast(
        "✓ Thank you! Your feedback has been sent directly to your coach & management.",
      );
      setFeedbackMessage("");
      setFeedbackRating(5);
    } catch (err) {
  ...
  const handleTrainerReviewSubmit = async (e) => {
    ...
    const fbPayload = {
      ...
      trainerId: user?.assignedTrainer || null,
      trainerName:
        user?.assignedTrainerName || myAssignedTrainer?.name || "Master Coach",
      athleteName: user?.name || "Gym Athlete",
      customerName: user?.name || "Gym Athlete",
      customerEmail: user?.email,
      customerAvatar:
    ...
    try {
      ...
      await api.post("/feedbacks", fbPayload);
      showToast(
        `✓ Thank you! Your review for ${user?.assignedTrainerName || myAssignedTrainer?.name || "Coach"} has been sent to their portal.`,
      );
      setTrainerReview("");
    } catch (err) {
  ...
  const handleGymReviewSubmit = async (e) => {
    ...
    try {
      ...
      await api.post("/feedbacks", fbPayload);
      showToast(
        "✓ Thank you! Your rating for Titan Pulse Gym has been recorded.",
      );
      setGymReview("");
    } catch (err) {
                    ...
                    <div className="pl-8 pr-2 py-1 space-y-0.5 border-l border-white/[0.08] ml-4 animate-fadeIn">
                      {sec.subsections.map((sub) => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${isSubActive ? "bg-[#FF1E27]" : "bg-slate-600"}`}
                            />
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Manage your athlete identity, contact information,
                        profile picture, and health telemetry.
                      </p>
                    </div>
                    <button
                  ...
                  <div className="p-6 sm:p-7 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm space-y-6">
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                        ...
                        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border-2 border-white/10 shadow-sm relative bg-[#0C0C10]">
                          ...
                          {uploadingAvatar && (
                            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-white text-[10px] gap-1.5 z-10">
                              <RefreshCw
                                size={20}
                                className="animate-spin text-[#FF1E27]"
                              />
                      ...
                      <div className="flex-1 text-center sm:text-left space-y-2">
                        ...
                        <p className="text-xs sm:text-sm text-slate-300">
                          ...
                          • Biometric Turnstile NFC Assigned
                        </p>
                        <p className="text-xs text-slate-400">
                          Member ID:{" "}
                          <span className="font-mono text-slate-300">
                            #TP-
                            {(user?._id || user?.id || "8842")
                              .slice(-6)
                              .toUpperCase()}
                          </span>{" "}
                          • Goal:{" "}
                            ...
                            <button
                              ...
                              onClick={() => {
                                const customUrl = window.prompt(
                                  "Paste direct image URL (or upload image file):",
                                  profilePic,
                                );
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        ...
                        <div className="p-4 rounded-xl bg-[#0D0D12] border border-white/[0.06] space-y-1">
                          ...
                          {isEditingProfile ? (
                            ...
                            <h4 className="text-lg font-bold text-[#FF1E27] font-['Outfit',sans-serif]">
                              {weight}
                            </h4>
                          )}
                        ...
                        <div className="p-4 rounded-xl bg-[#0D0D12] border border-white/[0.06] space-y-1">
                          ...
                          {isEditingProfile ? (
                            ...
                            <h4 className="text-lg font-bold text-amber-400 font-['Outfit',sans-serif]">
                              {bodyFat}
                            </h4>
                          )}
                        ...
                        <div className="p-4 rounded-xl bg-[#0D0D12] border border-white/[0.06] space-y-1">
                          ...
                          {isEditingProfile ? (
                            ...
                            <h4 className="text-lg font-bold text-cyan-400 font-['Outfit',sans-serif]">
                              {bloodGroup}
                            </h4>
                          )}
                    ...
                    <div className="pt-4 border-t border-white/[0.06] space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        >
                          <option value="Hypertrophy & Strength Progression">
                            Hypertrophy & Strength Progression
                          </option>
                          <option value="Fat Loss & Body Recomposition">
                            Fat Loss & Body Recomposition
                          </option>
                          <option value="Olympic Weightlifting & Power">
                            Olympic Weightlifting & Power
                          </option>
                          <option value="Cardio Conditioning & VO2 Max">
                            Cardio Conditioning & VO2 Max
                          </option>
                          <option value="Mobility, Flexibility & Longevity">
                            Mobility, Flexibility & Longevity
                          </option>
                        </select>
                      </div>
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Track your active membership passes, pre-workout orders,
                        wearables, and personal training sessions.
                      </p>
                    </div>
                            ...
                            <div className="flex items-start gap-4">
                              ...
                              <div className="space-y-1">
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className="font-mono font-semibold text-xs text-[#00F0FF]">
                                    {ord.id}
                                  </span>
                      ...
                      <div className="p-8 sm:p-10 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm text-center space-y-4 max-w-xl mx-auto">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
                            Active passes and merchandise purchases will appear
                            in this ledger once completed.
                          </p>
                        </div>
                        <button
              ...
              {activeSubTab === "password" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Manage your login credentials and secure your account
                      access.
                    </p>
                  </div>
                  ...
                  <div className="p-6 sm:p-7 rounded-2xl bg-[#121217] border border-white/[0.08] max-w-xl space-y-6 shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-semibold text-white font-['Outfit',sans-serif]">
                          Update Account Password
                        </h3>
                        <p className="text-xs text-slate-400">
                          Choose a secure password containing letters, numbers,
                          and symbols.
                        </p>
                      </div>
                    </div>
              ...
              {(activeSubTab === "logs" || !activeSubTab) && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-white/[0.08]">
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                        Gym Attendance & Turnstile Ledger
                      </h2>
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Real-time biometric turnstile check-ins, floor session
                        durations, and gate access logs.
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                    ...
                    <div className="p-5 rounded-2xl bg-[#121217] border border-white/[0.08] space-y-2 shadow-sm">
                      ...
                      <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, Math.round((attendanceRecords.length / 26) * 100))}%`,
                          }}
                        ></div>
                      </div>
                    </div>
              ...
              {activeSubTab === "analytics" && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-white/[0.08]">
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                        Workout Streak & Activity Density Graph
                      </h2>
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Interactive GitHub-style contribution matrix visualizing
                        workout volume, floor session frequency, and training
                        streaks.
                      </p>
                    </div>
                  ...
                  <div className="p-6 sm:p-8 rounded-3xl bg-[#121217] border border-white/[0.08] shadow-xl space-y-6">
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-white/[0.06]">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-400">
                          Hover over any node to inspect session duration,
                          target workout split, and date telemetry.
                        </p>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        ...
                        <div className="flex p-1 rounded-xl bg-[#090C0E] border border-white/10">
                          {[
                            ...
                            >
                              <span
                                className={`w-2 h-2 rounded-full ${v.color}`}
                              ></span>
                              <span>{v.label}</span>
                            </button>
                          ))}
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setStreakGraphAmbient(e.target.value)
                          }
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setStreakGraphAnimation(e.target.value)
                          }
                          className="px-3 py-1.5 rounded-xl bg-[#090C0E] border border-white/10 text-slate-300 text-xs outline-none focus:border-[#FF1E27] cursor-pointer"
                        >
            ...
            <div className="space-y-8 animate-fadeIn">
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Biometric turnstile access status, validity countdown,
                        and tier privileges.
                      </p>
                    </div>
                    {hasActiveMembership ? (
                    ...
                    <div className="p-6 sm:p-7 rounded-2xl bg-[#121217] border border-[#FF1E27]/30 shadow-md space-y-6 relative overflow-hidden">
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        ...
                        <div className="grid grid-cols-2 gap-3 w-full md:w-auto">
                          <div className="p-3.5 rounded-xl bg-[#0D0D12] border border-white/10">
                            ...
                            <span className="text-sm font-semibold text-white font-['Outfit',sans-serif]">
                              {membershipStartDate || "Active"}
                            </span>
                          </div>
                          <div className="p-3.5 rounded-xl bg-[#0D0D12] border border-white/10">
                            <span className="text-[11px] text-slate-400 uppercase tracking-wider block font-medium">
                            ...
                            <span className="text-sm font-semibold text-emerald-400 font-['Outfit',sans-serif]">
                      ...
                      <div className="space-y-2 pt-2">
                        ...
                        <div className="w-full h-2.5 rounded-full bg-black/60 border border-white/10 overflow-hidden p-0.5">
                          <div
                            ...
                            style={{
                              width: `${Math.max(5, remainingInfo.percentage)}%`,
                            }}
                          />
                        </div>
                      </div>
                      ...
                      <div className="pt-4 border-t border-white/[0.08] space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-[#FF1E27] font-semibold">
                            {currentPlanDetails?.tagline ||
                    ...
                    <div className="p-8 sm:p-10 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm text-center space-y-5 max-w-2xl mx-auto">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto leading-relaxed">
                          ...
                          linked to this account. Activate a tier to unlock 24/7
                          biometric turnstile gate access, lockers, sauna, and
                          coaching.
                        </p>
                      </div>
              ...
              {activeSubTab === "renew" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Extend your 24/7 biometric gym pass seamlessly with
                      instant checkout.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="p-6 rounded-2xl bg-[#121217] border border-white/[0.08] space-y-4 shadow-sm flex flex-col justify-between">
                      <div className="space-y-3">
                        ...
                        <h4 className="text-base font-semibold text-white font-['Outfit',sans-serif]">
                          Instant Biometric Sync
                        </h4>
                        <p className="text-xs text-slate-400 leading-relaxed">
                          Your RFID turnstile privileges update instantly on the
                          cloud biometric server and database upon successful
                      ...
                      <div className="p-4 rounded-xl bg-[#0D0D12] border border-white/[0.06] text-xs text-slate-300 space-y-1">
                        ...
                        <p className="text-slate-400">
                          Enter discount code at the reception desk for
                          corporate or group concessions.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {activeSubTab === "buy" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                      Buy / Upgrade Membership Tier
                    </h2>
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Explore our high-performance fitness tiers configured for
                      your training goals.
                    </p>
                  </div>
                    ...
                    {membershipPlans.map((plan) => {
                      const isCurrentPlan =
                        ...
                        ((membershipPlan &&
                          ...
                          (membershipPlan &&
                            ...
                            membershipPlan
                              .toLowerCase()
                              .includes(plan.tierKey.toLowerCase())) ||
                          (membershipPlan &&
                            plan.id &&
                            membershipPlan
                              .toLowerCase()
                              .includes(plan.id.toLowerCase())));

                      // Exactly one plan is selected at a time
                      const isSelected = selectedPlanId
                        ? selectedPlanId === plan.id ||
                          selectedPlanId === plan.tierKey ||
                          (plan.name &&
                      ...
                      return (
                        ...
                        >
                          {plan.badge && (
                            <span
                              className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-[10px] font-semibold uppercase shadow-sm ${
                                isSelected
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="pt-2 pb-1 border-y border-white/[0.06] flex items-baseline gap-1.5">
                              <span className="text-3xl sm:text-4xl font-bold text-white font-['Outfit',sans-serif] tracking-tight">
                                {plan.price}
                              </span>
                              ...
                              {plan.services && plan.services.length > 0 ? (
                                <div className="space-y-2">
                                  {plan.services.map((srv, idx) => (
                                    <div
                                      key={srv.id || idx}
                                      className="flex items-start justify-between gap-2 text-xs sm:text-sm py-0.5"
                                    >
                                      <div className="flex items-start gap-2 text-slate-200 min-w-0">
                                        ...
                                        <span
                                          className={`leading-relaxed text-xs sm:text-sm font-medium ${srv.included !== false ? "text-slate-200" : "text-slate-500 line-through"}`}
                                        >
                                          {srv.name}
                                        </span>
                                      </div>
                                ...
                                <div className="space-y-2">
                                  {plan.features.map((feat, idx) => (
                                    <div
                                      key={idx}
                                      className="flex items-start gap-2 text-xs sm:text-sm text-slate-200 py-0.5"
                                    >
                                      <Check
                                        size={15}
                                        className="text-[#FF1E27] shrink-0 mt-0.5"
                                      />
            ...
            <div className="space-y-8 animate-fadeIn">
              ...
              {(activeSubTab === "assigned" ||
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                        My Assigned Master Coach
                      </h2>
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Dedicated personal trainer assigned to manage your
                    ...
                    <div className="p-6 sm:p-8 rounded-3xl bg-[#121217] border border-white/[0.08] shadow-xl space-y-6 hover:border-[#FF1E27]/40 transition-all">
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 pb-6 border-b border-white/[0.06]">
                        <div className="flex items-center gap-4">
                          <img
                            src={
                              myAssignedTrainer.image ||
                              myAssignedTrainer.avatar ||
                              "https://images.unsplash.com/photo-1567013127542-490d757e51fc?auto=format&fit=crop&w=400&q=80"
                            }
                          ...
                          <div className="space-y-1">
                            ...
                            <span className="text-xs sm:text-sm text-[#FF1E27] font-semibold block">
                              {myAssignedTrainer.spec ||
                                "Master Strength & Hypertrophy Specialist"}
                            </span>
                            <span className="text-xs text-slate-400 block">
                              {myAssignedTrainer.experience ||
                                "6+ Years Experience"}{" "}
                              • Shift:{" "}
                              {myAssignedTrainer.shift || "06:00 AM - 02:00 PM"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                          <span className="px-3 py-1.5 rounded-xl bg-amber-500/10 text-amber-300 border border-amber-500/20 text-xs font-semibold flex items-center gap-1.5">
                            <Star
                              ...
                              className="fill-amber-400 text-amber-400"
                            />{" "}
                            {myAssignedTrainer.rating || "5.0 ★"} Rating
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                        <div className="p-4 rounded-2xl bg-[#090C0E] border border-white/5 space-y-1">
                          ...
                          <p className="font-semibold text-white">
                            {myAssignedTrainer.room || "Main Strength Arena"}
                          </p>
                        </div>
                        <div className="p-4 rounded-2xl bg-[#090C0E] border border-white/5 space-y-1">
                          <span className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold block">
                          ...
                          <p className="font-semibold text-emerald-400">
                            {membershipPlan &&
                            membershipPlan !== "No Active Plan"
                              ? `${membershipPlan} Routine`
                              : "Personalized Hypertrophy"}
                          </p>
                        </div>
                        <div className="p-4 rounded-2xl bg-[#090C0E] border border-white/5 space-y-1">
                          <span className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold block">
              ...
              {(activeSubTab === "all" || activeSubTab === "book") && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                      All Certified Faculty Trainers
                    </h2>
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Explore our certified master faculty specialists for
                      biomechanics, strength progression, and 1-on-1 personal
                      coaching.
                    </p>
                  </div>
                  ...
                  {realTrainers.length === 0 ? (
                    <div className="p-10 rounded-2xl bg-[#121217] border border-white/[0.08] text-center text-slate-400 text-xs">
                      {loadingTrainers
                        ? "Loading faculty trainers..."
                        : "No faculty trainers registered in the system."}
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {realTrainers.map((t) => {
                        const isAssignedToMe =
                          myAssignedTrainer &&
                          (myAssignedTrainer.id === t.id ||
                            myAssignedTrainer.name === t.name);
                        return (
                          <div
                            key={t.id}
              ...
              {activeSubTab === "previous" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                      Previous Coaches History
                    </h2>
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Historical log of master coaches previously assigned to
                      your training plans.
                    </p>
                  </div>
                  ...
                  <div className="p-10 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm text-center space-y-3 max-w-lg mx-auto">
                    ...
                    <h4 className="text-sm font-semibold text-white">
                      No Previous Coaching History
                    </h4>
                    <p className="text-xs text-slate-400">
                      When you complete coaching cycles with assigned trainers,
                      your past coach records will be archived here.
                    </p>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Unified ledger of all supplement purchases, merchandise,
                        membership passes, and registered transactions.
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      {
                        id: "all",
                        label: `All Transactions (${allTransactions.length})`,
                      },
                      {
                        id: "Supplements",
                        label: `Supplements & Gear (${supplementPayments.length})`,
                      },
                      {
                        id: "Membership",
                        label: `Membership Passes (${membershipPayments.length})`,
                      },
                    ].map((f) => (
                      <button
                        key={f.id}
                  ...
                  {filteredTransactions.length > 0 ? (
                      ...
                      <div className="overflow-x-auto w-full no-scrollbar">
                        <table className="min-w-[1020px] w-full text-left text-xs border-collapse">
                          <thead className="bg-[#181822] text-slate-400 text-xs font-semibold tracking-wider border-b border-white/[0.08]">
                            <tr>
                              <th className="px-4 py-3.5 font-semibold whitespace-nowrap min-w-[160px]">
                                Transaction ID
                              </th>
                              <th className="px-4 py-3.5 font-semibold whitespace-nowrap min-w-[120px]">
                                Date
                              </th>
                              <th className="px-4 py-3.5 font-semibold whitespace-nowrap min-w-[140px]">
                                Category
                              </th>
                              <th className="px-4 py-3.5 font-semibold min-w-[260px]">
                                Description / Items
                              </th>
                              <th className="px-4 py-3.5 font-semibold whitespace-nowrap min-w-[180px]">
                                Payment Method
                              </th>
                              <th className="px-4 py-3.5 font-semibold whitespace-nowrap min-w-[110px]">
                                Amount
                              </th>
                              <th className="px-4 py-3.5 font-semibold whitespace-nowrap min-w-[130px]">
                                Status
                              </th>
                              <th className="px-4 py-3.5 text-right font-semibold whitespace-nowrap min-w-[110px]">
                                Receipt
                              </th>
                            </tr>
                          </thead>
                            ...
                            {filteredTransactions.map((tx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3.5 text-slate-400 whitespace-nowrap align-middle">
                                    {tx.date}
                                  </td>
                                  <td className="px-4 py-3.5 whitespace-nowrap align-middle">
                                    <span
                                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider whitespace-nowrap ${
                                        isSupp
                                  ...
                                  <td
                                    className="px-4 py-3.5 font-medium text-white align-middle max-w-[280px]"
                                    title={tx.item}
                                  >
                                  ...
                                  <td className="px-4 py-3.5 text-slate-300 whitespace-nowrap align-middle">
                                    {tx.method}
                                  </td>
                                  <td className="px-4 py-3.5 font-bold text-emerald-400 font-mono text-sm whitespace-nowrap align-middle">
                                    {tx.amount}
                                  </td>
                                  <td className="px-4 py-3.5 whitespace-nowrap align-middle">
                                    <span
                    ...
                    <div className="p-8 sm:p-10 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm text-center space-y-4 max-w-xl mx-auto">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
                          No payments match the selected category. Complete a
                          supplement order or enroll in a membership pass to
              ...
              {activeSubTab === "membership" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Official membership subscriptions, validity billing
                      records, and downloadable tax invoices.
                    </p>
                  </div>
                  ...
                  {membershipPayments.length > 0 ? (
                    <div className="space-y-4">
                      {membershipPayments.map((mp) => (
                        ...
                        >
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <span className="font-mono text-xs text-[#FF1E27] font-semibold">
                                {mp.id}
                              </span>
                            ...
                            <p className="text-xs text-slate-400">
                              Billing Date: {mp.date} • Cycle: {mp.cycle} •
                              Method: {mp.method}
                            </p>
                    ...
                    <div className="p-8 sm:p-10 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm text-center space-y-4 max-w-xl mx-auto">
                      ...
                      <div className="space-y-1">
                        <h3 className="text-base sm:text-lg font-bold text-white font-['Outfit',sans-serif]">
                          No Membership Invoices Available
                        </h3>
                        <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
                          Official GST tax invoices are generated automatically
                          upon membership enrollment or renewal.
                        </p>
                      </div>
                      <button
                        onClick={() => handleTabChange("membership", "buy")}
              ...
              {activeSubTab === "supplements" && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                        Supplements & Store Orders
                      </h2>
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Official purchase receipts for nutritional supplements,
                        protein isolate, pre-workout, and fitness accessories.
                      </p>
                    </div>
                    <Link
                      to="/my-cart"
                  ...
                  {supplementPayments.length > 0 ? (
                    ...
                    <div className="p-8 sm:p-10 rounded-2xl bg-[#121217] border border-white/[0.08] shadow-sm text-center space-y-4 max-w-xl mx-auto">
                      ...
                      <div className="space-y-1">
                        <h3 className="text-base sm:text-lg font-bold text-white font-['Outfit',sans-serif]">
                          No Supplement Purchases Yet
                        </h3>
                        <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
                          Your orders for pre-workouts, hydrolyzed whey isolate,
                          creatine, and gym accessories will appear here once
                          purchased.
                        </p>
                      </div>
                      <Link
          ...
          {activeTab === "workout-diet" && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                    Master Coach Protocol & Telemetry Hub
                  </h2>
                  <p className="text-xs sm:text-sm text-slate-400 mt-1">
                    Live workout splits, macro nutrition protocols, coach
                    observation logs, PR telemetry, and 1-on-1 direct coaching
                    stream.
                  </p>
                </div>
                {user?.assignedTrainerName && (
                  <span className="px-3.5 py-1.5 rounded-xl bg-[#FF1E27]/10 border border-[#FF1E27]/30 text-[#FF1E27] text-xs font-mono font-semibold flex items-center gap-1.5">
                    <UserCheck size={14} className="text-[#FF1E27]" /> Assigned
                    Coach: {user.assignedTrainerName}
                  </span>
                )}
              </div>
              ...
              <div className="bg-[#12141C] border border-white/[0.08] p-1.5 rounded-2xl flex items-center gap-2 overflow-x-auto no-scrollbar shadow-md">
                {[
                  ...
                  const Icon = sub.icon;
                  const isSubActive =
                    coachingSubTab === sub.id ||
                    activeSubTab === sub.id ||
                    (!coachingSubTab && sub.id === "workout-plan");
                  return (
                    <button
                      key={sub.id}
                      onClick={() => {
              ...
              {(coachingSubTab === "workout-plan" ||
                activeSubTab === "workout-plan" ||
                (!coachingSubTab && !activeSubTab)) && (
                  ...
                  <div className="flex items-center justify-between pb-4 border-b border-white/[0.06]">
                    <div>
                      <h4 className="text-lg font-bold text-white font-['Outfit',sans-serif] flex items-center gap-2">
                        <Dumbbell size={20} className="text-[#FF1E27]" />{" "}
                        Prescribed Workout Protocol
                      </h4>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Biomechanical hypertrophy & strength progression
                        prescribed by your assigned coach.
                      </p>
                    </div>
                    <span className="text-[11px] text-slate-400 font-mono">
                      Last update:{" "}
                      {coachingData.workoutPlan?.updatedAt ||
                        "Recently by Coach"}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-white text-sm">
                        {coachingData.workoutPlan?.split ||
                    ...
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-[#FF1E27] text-sm">
                        {coachingData.workoutPlan?.frequency || "5 Days / Week"}
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-white text-sm">
                        {coachingData.workoutPlan?.intensity ||
                    ...
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-emerald-400 text-sm">
                        {coachingData.workoutPlan?.cardioProtocol ||
                  ...
                  {coachingData.workoutPlan?.customNotes && (
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-[#FF1E27]/20 space-y-1 text-xs">
                      <span className="text-[#FF1E27] font-bold uppercase tracking-wider text-[10px] flex items-center gap-1.5">
                        <NotebookPen size={13} /> COACH TECHNICAL FORM CUES &
                        EXECUTION NOTES
                      </span>
                    ...
                    {activeWorkoutSplits[safeWorkoutDay] && (
                      <div className="p-5 sm:p-6 rounded-2xl bg-[#090A0E] border border-white/[0.08] space-y-4 shadow-sm">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-3 border-b border-white/[0.06]">
                          <div>
                            <h4 className="text-base font-semibold text-white font-['Outfit',sans-serif]">
                              {activeWorkoutSplits[safeWorkoutDay].title}
                            </h4>
                            <p className="text-xs text-[#FF1E27] font-semibold mt-0.5">
                              {activeWorkoutSplits[safeWorkoutDay].focus}
                            </p>
                        ...
                        <div className="space-y-2.5">
                          ...
                          activeWorkoutSplits[safeWorkoutDay].exercises
                            ...
                            activeWorkoutSplits[safeWorkoutDay].exercises.map(
                              (ex, idx) => {
                                const isDone =
                                  completedExercises[ex.id || `ex-${idx}`];
                                return (
                                  <div
                                    key={ex.id || idx}
                                    onClick={() =>
                                      toggleExercise(ex.id || `ex-${idx}`)
                                    }
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                        <h5
                                          className={`text-xs sm:text-sm font-semibold ${isDone ? "line-through text-slate-400" : "text-white"}`}
                                        >
                                          {ex.name}
                                        </h5>
              ...
              {(coachingSubTab === "diet-plan" ||
                ...
                <div className="p-6 sm:p-8 rounded-3xl bg-[#12141C] border border-white/[0.08] shadow-xl space-y-6 animate-fadeIn">
                  <div className="flex items-center justify-between pb-4 border-b border-white/[0.06]">
                    <div>
                      <h4 className="text-lg font-bold text-white font-['Outfit',sans-serif] flex items-center gap-2">
                        ...
                        Prescribed Macro & Nutrition Protocol
                      </h4>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Caloric targets, macro ratios, hydration goals, and meal
                        timing assigned by your coach.
                      </p>
                    </div>
                    <span className="text-[11px] text-slate-400 font-mono">
                      Last update:{" "}
                      {coachingData.dietPlan?.updatedAt || "Recently by Coach"}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="p-4 sm:p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1 shadow-sm">
                      ...
                      <h4 className="text-lg sm:text-xl font-bold text-white flex items-center gap-1.5 font-['Outfit',sans-serif]">
                        <Flame size={18} className="text-[#FF1E27]" />{" "}
                        {coachingData.dietPlan?.dailyCalories || "2,800 kcal"}
                      </h4>
                      <span className="text-xs text-slate-400 block">
                        Daily Energy Burn
                      </span>
                    ...
                    <div className="p-4 sm:p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1 shadow-sm">
                      ...
                      <span className="text-xs text-amber-400/80 block">
                    ...
                    <div className="p-4 sm:p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1 shadow-sm">
                      ...
                      <span className="text-xs text-cyan-400/80 block">
                        Hormonal Balance
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-2">
                      ...
                      <div className="bg-[#12141C] p-3.5 rounded-xl border border-white/5 space-y-1.5">
                        {Array.isArray(coachingData.dietPlan?.supplements) ? (
                          coachingData.dietPlan.supplements.map(
                            (supp, sIdx) => (
                              <div
                                key={sIdx}
                          ...
                          <p className="text-slate-300 text-xs sm:text-sm">
                            {coachingData.dietPlan?.supplements ||
                              "Whey Isolate, Creatine Creapure, Electrolytes"}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="p-5 sm:p-6 rounded-2xl bg-[#090A0E] border border-white/5 flex items-center justify-between gap-4 shadow-sm">
                    <div className="space-y-1">
                      <h4 className="text-sm sm:text-base font-semibold text-white font-['Outfit',sans-serif]">
                        Daily Hydration Log: {(waterGlasses * 0.25).toFixed(2)}L
                        /{" "}
                        {coachingData.dietPlan?.waterIntake ||
                          "4.0 Liters Target"}
                      </h4>
                      <p className="text-xs text-slate-400">
                        Target assigned by coach:{" "}
                        {coachingData.dietPlan?.waterIntake ||
                          "4.0 Liters Daily"}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-cyan-400 px-2">
              ...
              {(coachingSubTab === "trainer-notes" ||
                activeSubTab === "trainer-notes") && (
                <div className="p-6 sm:p-8 rounded-3xl bg-[#12141C] border border-white/[0.08] shadow-xl space-y-6 animate-fadeIn">
                  <div className="pb-4 border-b border-white/[0.06]">
                    <h4 className="text-lg font-bold text-white font-['Outfit',sans-serif] flex items-center gap-2">
                      <NotebookPen size={20} className="text-[#FF1E27]" /> Coach
                      Observation & Form Audit Notes
                    </h4>
                    <p className="text-xs text-slate-400 mt-0.5">
                      Chronological coaching observations, form corrections, and
                      progress notes written by your trainer.
                    </p>
                  </div>

                  <div className="space-y-3">
                    {!coachingData.trainerNotes ||
                    coachingData.trainerNotes.length === 0 ? (
                      <div className="p-8 rounded-2xl bg-[#090A0E] text-center text-slate-400 text-xs">
                        No coach observation notes logged yet. When your trainer
                        logs advice, it will appear here.
                      </div>
                    ) : (
                      coachingData.trainerNotes.map((n, i) => (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <span className="font-bold text-[#FF1E27] font-mono">
                              {n.author ||
                                user?.assignedTrainerName ||
                                "Master Coach"}
                            </span>
              ...
              {(coachingSubTab === "progress" ||
                activeSubTab === "progress") && (
                <div className="p-6 sm:p-8 rounded-3xl bg-[#12141C] border border-white/[0.08] shadow-xl space-y-6 animate-fadeIn">
                  <div className="flex items-center justify-between pb-4 border-b border-white/[0.06]">
                    <div>
                      <h4 className="text-lg font-bold text-white font-['Outfit',sans-serif] flex items-center gap-2">
                        <LineChart size={20} className="text-[#FF1E27]" />{" "}
                        Athlete Biometrics & 1RM PR Telemetry
                      </h4>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Verified compound 1-Rep-Max strength records and body
                        composition tracked with your trainer.
                      </p>
                    </div>
                    <span className="text-[11px] text-slate-400 font-mono">
                      Last audit:{" "}
                      {coachingData.progress?.lastAuditDate || "30 Aug 2026"}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <div className="p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-2">
                      ...
                      <div className="text-2xl font-bold text-white font-mono">
                        {coachingData.progress?.benchPressPR || "110 kg"}
                      </div>
                      <span className="text-[10px] text-[#FF1E27] font-mono">
                        Verified Compound Max
                      </span>
                    </div>
                    ...
                    <div className="p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-2">
                      ...
                      <div className="text-2xl font-bold text-white font-mono">
                        {coachingData.progress?.squatPR || "150 kg"}
                      </div>
                      <span className="text-[10px] text-[#FF1E27] font-mono">
                        Verified Compound Max
                      </span>
                    ...
                    <div className="p-5 rounded-2xl bg-[#090A0E] border border-white/5 space-y-2">
                      ...
                      <div className="text-2xl font-bold text-white font-mono">
                        {coachingData.progress?.deadliftPR || "190 kg"}
                      </div>
                      <span className="text-[10px] text-[#FF1E27] font-mono">
                        Verified Compound Max
                      </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 text-xs">
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-white text-base font-mono">
                        {coachingData.progress?.currentWeight ||
                          weight ||
                          "76 kg"}
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-emerald-400 text-base font-mono">
                        {coachingData.progress?.targetWeight ||
                          "80 kg Lean Mass"}
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-2xl bg-[#090A0E] border border-white/5 space-y-1">
                      ...
                      <p className="font-bold text-[#FF1E27] text-base font-mono">
                        {coachingData.progress?.bodyFat || bodyFat || "14.2%"}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {(coachingSubTab === "chat" || activeSubTab === "chat") && (
                <div className="p-6 sm:p-8 rounded-3xl bg-[#12141C] border border-white/[0.08] shadow-xl space-y-4 animate-fadeIn">
                  <div className="flex items-center justify-between pb-3 border-b border-white/[0.06]">
                    <div>
                      <h4 className="text-lg font-bold text-white font-['Outfit',sans-serif] flex items-center gap-2">
                        <MessageSquare size={20} className="text-[#FF1E27]" />{" "}
                        1-on-1 Coach & Athlete Advisory Channel
                      </h4>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Direct messaging channel with{" "}
                        {user?.assignedTrainerName ||
                          myAssignedTrainer?.name ||
                          "Master Coach Vikram"}
                        .
                  ...
                  <div className="flex flex-col h-[480px] rounded-2xl bg-[#090A0E] border border-white/10 overflow-hidden">
                    <div className="flex-1 p-5 overflow-y-auto custom-scrollbar space-y-3">
                      {customerChatMessages.length === 0 ? (
                        <div className="h-full flex items-center justify-center text-slate-500 text-xs">
                          No previous messages with your coach. Type your
                          question or progress update below.
                        </div>
                      ) : (
                    ...
                    <form
                      onSubmit={handleCustomerSendChat}
            ...
            <div className="space-y-8 animate-fadeIn">
              ...
              {(activeSubTab === "submit" || !activeSubTab) && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Help us elevate Titan Pulse facilities, training coaching,
                      and turnstile operations.
                    </p>
                  </div>

                  <form
                    onSubmit={handleFeedbackSubmit}
                    className="p-6 sm:p-8 rounded-2xl bg-[#121217] border border-white/[0.08] space-y-5 shadow-sm max-w-2xl"
                  >
                    <div className="space-y-2">
                      <label className="text-xs font-medium text-slate-300">
                        Overall Rating
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        <option value="Facility & Equipment">
                          Facility & Equipment Maintenance
                        </option>
                        <option value="Biometric Gate Pass">
                          Biometric Speed Gates & Turnstiles
                        </option>
                        <option value="Trainer Consultation">
                          Trainer Consultation & Workout Splits
                        </option>
                        <option value="Cleanliness & Hygiene">
                          Cleanliness, Shower & Sauna Hygiene
                        </option>
                        <option value="General Suggestion">
                          General Suggestion / Feature Request
                        </option>
                      </select>
                    </div>
              ...
              {activeSubTab === "rate" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight font-['Outfit',sans-serif]">
                      Dedicated Ratings: Trainer & Gym Ambiance
                    </h2>
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Rate your assigned personal trainer and overall facility
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="p-6 rounded-2xl bg-[#121217] border border-white/[0.08] space-y-4 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-semibold text-white font-['Outfit',sans-serif]">
                            Rate Titan Pulse Facility
                          </h4>
                          <span className="text-xs text-slate-400">
                            Equipment, Hygiene & Ambiance
                          </span>
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-4 border-b border-white/[0.08]">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-slate-400 mt-1">
                        Track resolution status on any inquiries or maintenance
                        issues submitted to Management.
                      </p>
                    </div>
                    <button
                    ...
                    {supportTickets.length === 0 ? (
                      <div className="p-8 sm:p-12 rounded-2xl bg-[#121217] border border-white/[0.08] text-center space-y-3">
                        <p className="text-xs sm:text-sm text-slate-400">
                          No support tickets or complaints logged yet.
                        </p>
                        <button
                          onClick={() => setTicketModalOpen(true)}
              ...
              {activeSubTab === "faq" && (
                <div className="space-y-6">
                  <div className="pb-4 border-b border-white/[0.08]">
                    ...
                    <p className="text-xs sm:text-sm text-slate-400 mt-1">
                      Quick answers to biometric gates, lockers, and emergency
                      hotlines.
                    </p>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <ChevronDown
                              size={15}
                              className={`transition-transform ${isOpen ? "rotate-180 text-[#FF1E27]" : "text-slate-400"}`}
                            />
                          </button>
                          {isOpen && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
                    <div className="p-4 sm:p-5 rounded-xl bg-[#121217] border border-white/[0.08] space-y-1.5">
                      ...
                      <h4 className="text-xs font-semibold text-white font-['Outfit',sans-serif]">
                        Front Desk Hotline
                      </h4>
                      <p className="text-xs text-slate-400 font-mono">
                        +91 98765 00123
          ...
          <div className="relative w-full max-w-md bg-[#121217] rounded-3xl border border-white/20 shadow-2xl animate-fadeIn flex flex-col h-[500px] overflow-hidden">
            ...
            <div className="p-4 bg-[#181822] border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img
                  src={
                    chatModalTrainer.image ||
                    chatModalTrainer.avatar ||
                    "https://images.unsplash.com/photo-1567013127542-490d757e51fc?auto=format&fit=crop&w=400&q=80"
                  }
                ...
                />
                <div>
                  <h4 className="text-xs sm:text-sm font-semibold text-white font-['Outfit',sans-serif]">
                    {chatModalTrainer.name}
                  </h4>
                  <span className="text-[10px] text-emerald-400">
                    ● Online on Titan Athlete Network
                  </span>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 p-4 space-y-3 overflow-y-auto text-xs sm:text-sm">
              <div className="flex flex-col items-start max-w-[80%]">
                <div className="p-3 rounded-2xl bg-white/[0.06] text-slate-200 rounded-tl-none leading-relaxed">
                  Hey {firstName}! I reviewed your chest workout telemetry from
                  yesterday. Great 85kg bench press form!
                </div>
              ...
              <div className="flex flex-col items-end max-w-[80%] ml-auto">
                <div className="p-3 rounded-2xl bg-[#FF1E27] text-white rounded-tr-none leading-relaxed">
                  Thanks Coach! Ready for tomorrow's deadlift session. Should I
                  increase target to 150kg?
                </div>
                <span className="text-[10px] text-slate-500 mt-0.5">
                  10:18 AM
                </span>
              ...
              <div className="flex flex-col items-start max-w-[80%]">
                <div className="p-3 rounded-2xl bg-white/[0.06] text-slate-200 rounded-tl-none leading-relaxed">
                  Let's do 145kg for 4 sets of 5 reps first to protect spinal
                  bracing. See you on the floor at 07:00 AM!
                </div>
                <span className="text-[10px] text-slate-500 mt-0.5">
                  10:20 AM
                </span>
        ...
        <div className="fixed inset-0 z-[120] bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative w-full max-w-md bg-[#121217] rounded-3xl border border-[#FF1E27]/40 p-6 shadow-2xl animate-fadeIn space-y-4">
            ...
            <div className="pb-2 border-b border-white/10">
              <h3 className="text-base font-bold text-white font-['Outfit',sans-serif]">
                Raise Support / Service Ticket
              </h3>
              <p className="text-xs text-slate-400">
                Directly dispatched to Titan Pulse Front Desk & Maintenance Ops.
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="Biometric Speed Gate">
                      Biometric Speed Gate
                    </option>
                    <option value="Billing & Invoices">
                      Billing & Invoices
                    </option>
                    <option value="Coach Consultation">
                      Coach Consultation
                    </option>
                  </select>
                </div>
          ...
          <div className="relative w-full max-w-lg bg-[#121218] rounded-3xl border border-white/20 shadow-[0_0_50px_rgba(0,0,0,0.9)] animate-fadeIn overflow-hidden flex flex-col">
            ...
            <div className="p-5 bg-gradient-to-r from-[#181824] to-[#12121A] border-b border-white/10 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    <h3 className="text-sm font-bold text-white tracking-wide font-['Outfit',sans-serif]">
                      TITAN PULSE CHECKOUT
                    </h3>
              ...
              <div className="text-right pr-6">
                ...
                <span className="text-xl font-bold font-mono text-[#FF1E27]">
                  ₹
                  {Number(paymentModalData.priceNum || 2499).toLocaleString(
                    "en-IN",
                  )}
                </span>
              </div>
            ...
            <div className="p-6 space-y-5 bg-[#0D0D14]">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-xs font-bold text-white">
                        Pay with Cash at Front Desk
                      </h4>
                      <p className="text-[11px] text-slate-300 leading-relaxed">
                        Pay cash directly to the receptionist at the gym front
                        entrance desk. Your RFID biometric turnstile pass will
                        be activated upon token submission.
                      </p>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-center gap-2 text-[10px] text-slate-400">
                ...
                <span>
                  Instant Biometric Activation • Official Tax Invoice Generated
                </span>
              </div>
            </div>
          </div>

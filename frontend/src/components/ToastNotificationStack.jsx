...
import { CheckCircle2, AlertCircle, Info, Bell, X, Zap, Flame, ShieldCheck } from "lucide-react";
...
export default function ToastNotificationStack({
  ...
  const positionClasses = {
    "top-right": "fixed top-6 right-6 z-[9999] max-w-sm w-full pointer-events-none",
    "top-center": "fixed top-6 left-1/2 -translate-x-1/2 z-[9999] max-w-md w-full pointer-events-none",
    "bottom-right": "fixed bottom-6 right-6 z-[9999] max-w-sm w-full pointer-events-none",
  };
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "gate":
      case "zap":
        return <Zap size={18} className="text-[#FF1E27] shrink-0 fill-[#FF1E27]" />;
      case "streak":
        return <Flame size={18} className="text-amber-400 shrink-0 fill-amber-400" />;
      case "security":
  ...
  return (
    <div className={positionClasses[position] || positionClasses["top-right"]}>
      <AnimatedList
        ...
        renderItem={(item) => (
          <div
            className="pointer-events-auto flex items-start gap-3 p-4 rounded-2xl bg-[#12151F]/95 border border-white/15 backdrop-blur-2xl shadow-[0_12px_40px_rgba(0,0,0,0.8)] text-white group relative overflow-hidden"
          >

  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              ...
              <div className="z-10">
                ...
                <p className="text-xs font-mono text-[#00F0FF] mt-1 mb-2">
                  {item.subtitle}
                </p>
                <p className="text-xs text-[#8A94A0] leading-relaxed line-clamp-2">
                  {item.desc}
                </p>
              </div>
            </div>
      ...
      <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 z-40 flex items-center gap-3 bg-[#090C0E]/90 border border-white/10 px-4 py-1.5 rounded-full backdrop-blur-md">
        ...
        <button
          onClick={() =>
            onSelectIndex(Math.min(items.length - 1, activeIndex + 1))
          }
...
export default function LetsDrive() {
  ...
  const stepsData =
    eqData.steps && eqData.steps.length > 0 ? eqData.steps : DEFAULT_STEPS;

  const [activeStep, setActiveStep] = useState(0);
  const [videoModalOpen, setVideoModalOpen] = useState(false);
  const [selectedVideo, setSelectedVideo] = useState(
    stepsData[0] || DEFAULT_STEPS[0],
  );
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3.5">
                    <span
                      className={`w-9 h-9 rounded-full border flex items-center justify-center font-mono text-xs font-bold transition-colors ${
                        activeStep === index
                  ...
                  <ChevronDown
                    className={`w-5 h-5 text-[#8A94A0] transition-transform duration-300 ${activeStep === index ? "rotate-180 text-[#FF2E4C]" : ""}`}
                  />
                </div>

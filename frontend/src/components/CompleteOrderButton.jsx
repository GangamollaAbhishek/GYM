...
export default function CompleteOrderButton({
  ...
  const setTruckTransition = (seconds, easing = 'cubic-bezier(0.25, 1, 0.5, 1)') => {
    if (truckRef.current) {
  ...
  const handleClick = async (e) => {
    ...
    setTruckTransition(BASE_TIMINGS.REVERSE_LOAD / 1000, 'cubic-bezier(0.4, 0, 0.2, 1)');
    if (truckRef.current) {
    ...
    <div className={`complete-order-container ${className}`}>
      ...
      >
        ...
        <span className="btn-text default-text">
          ...
          <span>{label} {amountText ? `• ${amountText}` : ''}</span>
        </span>
        ...
        <div ref={truckRef} className={`truck-wrapper ${truckState}`}>
          <svg className="truck-svg" viewBox="0 0 140 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M72 4 H88 C94 4 98 9 98 24 C98 39 94 44 88 44 H72 V4 Z" fill="#FF1E27" />
